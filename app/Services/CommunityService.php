...
class CommunityService
{
    ...
    public function removeMember(Request $request, $communityId)
    {
        ...
        // Handle single ID, array of IDs, or comma-separated string
        $removeUserIds = $request->remove_user_id;
        if (is_string($removeUserIds) && strpos($removeUserIds, ',') !== false) {
            $removeUserIds = array_map('trim', explode(',', $removeUserIds));
        } elseif (!is_array($removeUserIds)) {
            $removeUserIds = [$removeUserIds];
        }
        
        // Ensure all IDs are strings for reliable comparison
        $removeUserIds = array_map('strval', $removeUserIds);
        ...
        $communityUsers = array_filter($communityUsers, function ($uid) use ($removeUserIds) {
            return !in_array((string)$uid, $removeUserIds);
        });
        $communityAdmins = array_filter($communityAdmins, function ($uid) use ($removeUserIds) {
            return !in_array((string)$uid, $removeUserIds);
        });
        ...
        foreach ($communityGroups as $groupId) {
            ...
            if ($group) {
                ...
                $groupUsers = array_filter($groupUsers, function ($uid) use ($removeUserIds) {
                    return !in_array((string)$uid, $removeUserIds);
                });
                $groupAdmins = array_filter($groupAdmins, function ($uid) use ($removeUserIds) {
                    return !in_array((string)$uid, $removeUserIds);
                });
