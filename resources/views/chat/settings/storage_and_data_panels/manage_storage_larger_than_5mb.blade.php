<div id="manage_storage_larger_than_5mb_panel" class="hidden flex-col w-full sm:w-[30%] sm:min-w-[350px] border-r border-[#313d45] bg-[#111b21] h-full shrink-0 overflow-hidden relative">
    
    <!-- Default Header -->
    <div id="larger_5mb_default_header" class="h-16 bg-[#202c33] px-6 flex items-center justify-between shrink-0 border-b border-[#313d45] transition-all">
        <div class="flex items-center gap-6">
            <button onclick="toggleLargerThan5MB()" class="text-[#aebac1] hover:text-white transition-colors">
                <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
                    <path d="M12 4l1.4 1.4L7.8 11H20v2H7.8l5.6 5.6L12 20l-8-8 8-8z"></path>
                </svg>
            </button>
            <div class="flex flex-col">
                <h2 class="text-[#e9edef] text-[19px] font-semibold">Larger than 5 MB</h2>
                <span class="text-[#8696a0] text-[13px]">8.0 GB</span>
            </div>
        </div>
        <button onclick="toggleSortModal()" class="text-[#aebac1] hover:text-white transition-colors">
            <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
    ...
    <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar bg-[#111b21]">
        <div class="grid grid-cols-3 gap-0.5" id="larger_5mb_grid">
            <!-- Dynamic Grid Items Injected Here -->
        </div>
    </div>

    <!-- Sort Modal (Bottom Sheet) -->
    <div id="manage_storage_sort_modal" class="hidden absolute inset-0 z-50 flex-col justify-end bg-black/50 transition-opacity">
        <div class="bg-[#233138] w-full rounded-t-2xl py-4 flex flex-col transform translate-y-full transition-transform duration-300" id="sort_modal_sheet">
            <div class="px-6 pb-2 mb-2">
                <span class="text-[#8696a0] text-[15px]">Sort by</span>
            </div>
            <div class="flex items-center justify-between px-6 py-3 cursor-pointer hover:bg-[#182229] transition-colors" onclick="selectSort('Newest')">
            ...
            <div class="flex items-center justify-between px-6 py-3 cursor-pointer hover:bg-[#182229] transition-colors" onclick="selectSort('Oldest')">
                <span class="text-[#e9edef] text-[16px]">Oldest</span>
                <svg class="sort-check hidden w-5 h-5 text-[#00a884]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
            </div>
            <div class="flex items-center justify-between px-6 py-3 cursor-pointer hover:bg-[#182229] transition-colors" onclick="selectSort('Largest')">
                <span class="text-[#e9edef] text-[16px]">Largest</span>
                <svg id="sort_largest_check" class="sort-check w-5 h-5 text-[#00a884]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
            </div>
        </div>
    </div>

    <!-- Custom Delete Modal -->
    <div id="storage_delete_modal" class="hidden absolute inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm transition-all duration-300">
        <div class="bg-[#3b4a54] w-[90%] max-w-[340px] rounded-[3px] p-6 shadow-2xl transform scale-95 transition-all duration-300 opacity-0" id="storage_delete_modal_content">
...
<script>
    ...
    // Expose render function globally so manage_storage can trigger it when async data loads
    window.renderGrid = renderGrid;

    document.addEventListener('DOMContentLoaded', () => {
        renderGrid();
        
        // Setup Sort Modal Close on outside click
        document.getElementById('manage_storage_sort_modal').addEventListener('click', (e) => {
            if(e.target.id === 'manage_storage_sort_modal') {
                closeSortModal();
            }
        });
    });
    ...
    function renderGrid() {
        ...
        const grid = document.getElementById('larger_5mb_grid');
        let html = '';
        
        // Apply Sort
        let sortedItems = [...mockMediaItems];
    ...
    function executeStorageDeletion() {
        ...
        Promise.all(promises).then(() => {
            ...
            if (window.fullStorageCache) {
                window.fullStorageCache = window.fullStorageCache.filter(m => !selectedItemIds.has(m.key));
            }
            if (window.globalMediaCache) {
                window.globalMediaCache = window.globalMediaCache.filter(m => !selectedItemIds.has(m.key));
            }
    ...
    function toggleItemSelection(id) {
        ...
        if (selectedItemIds.size === mockMediaItems.length && mockMediaItems.length > 0) {
    ...
    function updateSelectionHeader() {
        ...
        if (selectedItemIds.size > 0) {
            ...
            document.getElementById('selection_count_title').innerText = selectedItemIds.size;
    ...
    function toggleSortModal() {
        const modal = document.getElementById('manage_storage_sort_modal');
        const sheet = document.getElementById('sort_modal_sheet');
