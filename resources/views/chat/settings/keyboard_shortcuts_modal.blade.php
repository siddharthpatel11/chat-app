            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-7">
                @foreach($shortcuts as $shortcut)
                    <div class="flex items-center justify-between">
                        <span class="text-[#e9edef] text-[15px] truncate mr-2">{{ $shortcut['label'] }}</span>
                        <div class="flex gap-1.5 shrink-0">
                            @foreach($shortcut['keys'] as $key)
                                <kbd class="px-2 py-0.5 bg-[#111b21] text-[#aebac1] rounded-lg text-[13px] border border-[#313d45] shadow-sm">{{ $key }}</kbd>
                            @endforeach
                        </div>
                    </div>
                @endforeach
