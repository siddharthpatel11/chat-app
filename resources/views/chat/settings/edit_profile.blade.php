        ...
        <div class="flex flex-col items-center py-10 px-6">
            <div class="relative group">
                <div class="w-[200px] h-[200px] rounded-full overflow-hidden shadow-2xl border-4 border-[#313d45] cursor-pointer"
                    onclick="viewPhoto()">
                    <img src="{{ auth()->user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=2a3942&color=fff&size=256' }}"
        ...
        <div class="flex flex-col px-8 pb-10">
                ...
                <div id="about_display_mode"
                    ...
                    <div class="flex flex-col">
                        <span class="text-[#e9edef] text-[17px] font-normal"
                            id="profile_about_text">{{ auth()->user()->about ?? auth()->user()->name }}</span>
                        <span class="text-[#8696a0] text-[14px] mt-1 italic"
                            id="profile_about_subtitle">{{ auth()->user()->about_subtitle ?? 'Until I change it' }}</span>
                    </div>
            ...
            <div class="mb-10 group">
                ...
                <div id="name_display_mode"
                    ...
                    <span class="text-[#e9edef] text-[17px] font-normal"
                        id="profile_name_text">{{ auth()->user()->name }}</span>
                    <button class="text-[#8696a0] hover:text-[#e9edef] transition-colors">
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                            <path
                                d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z">
...
<div id="view_photo_modal"
    ...
    <div
        ...
        <div class="flex items-center gap-4">
            ...
            <span class="text-white font-medium settings-profile-name">{{ auth()->user()->name }}</span>
        </div>
...
<script>
    ...
    const defaultAvatar = "https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=2a3942&color=fff&size=256";
