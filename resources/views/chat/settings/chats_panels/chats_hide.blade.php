<div id="chats_hide_panel"
    ...
    <div class="h-16 bg-[#202c33] px-6 flex items-center justify-between shrink-0 border-b border-[#313d45]">
        ...
        <div class="relative">
            <button id="hide_chat_menu_btn" onclick="toggleHideChatHeaderMenu(event)"
                class="text-[#aebac1] hover:text-white transition-colors focus:outline-none hidden">
            ...
            <!-- Dropdown -->
            <div id="hide_chat_header_dropdown"
                class="hidden absolute right-0 top-10 w-44 bg-[#233138] rounded-lg shadow-xl border border-[#313d45] py-2 z-50 transform origin-top-right transition-all">
                <button onclick="window.openChangePasswordModal(event)"
                    class="w-full text-left px-4 py-2.5 text-[#e9edef] hover:bg-[#182229] transition-colors text-sm">
                    Change Password
                </button>
            </div>
        </div>
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar bg-[#111b21] py-4 px-6 flex flex-col h-full relative z-10">
        ...
        <div id="hide_chat_state_setup" class="hidden flex flex-col gap-4">
            ...
            <p class="text-[#8696a0] text-[13px]">Create a password to secure your hidden chats. This password is
                separate from your App Lock.</p>
            ...
            <div class="space-y-3 mt-2">
                ...
                <p id="hide_chat_setup_error" class="text-red-500 text-xs hidden">Passwords do not match or are too
                    short.</p>
        ...
        <div id="hide_chat_state_unlock" class="hidden flex flex-col gap-4">
            ...
            <p class="text-[#8696a0] text-[13px] text-center">Enter your hide chat password to manage or view hidden
                chats.</p>
        ...
        <div id="hide_chat_state_manage" class="hidden flex flex-col gap-6">
            ...
            <!-- Hide a new chat -->
            <div
                class="flex flex-col gap-3 bg-[#202c33]/40 p-4 rounded-xl border border-white/5 max-h-[300px] shrink-0">
                <h4 class="text-[#00a884] text-xs font-semibold uppercase tracking-wider">Add to Hide Chat</h4>

                <!-- Search input -->
                <div
                    class="bg-[#202c33] rounded-lg flex items-center px-3 h-9 border border-transparent focus-within:border-[#00a884]/30 shrink-0">
                    ...
                    <input type="text" id="hide_chat_search_input" oninput="window.filterChatsToHide()"
                        placeholder="Search chats"
                        class="bg-transparent border-none focus:ring-0 w-full text-xs ml-2 text-[#d1d7db] placeholder-[#8696a0] focus:outline-none">
                </div>

                <!-- Scrollable list of available chats -->
                <div id="chats_to_hide_list"
                    class="flex-grow overflow-y-auto custom-scrollbar space-y-1.5 max-h-[200px]">
...
<div id="hide_chat_change_pwd_modal"
    ...
    <div class="bg-[#233138] w-[90%] max-w-[340px] rounded-2xl p-6 shadow-2xl transform scale-95 transition-all duration-300 opacity-0"
        id="hide_chat_change_pwd_content">
...
<div id="hidden_chat_click_unlock_modal"
    class="hidden fixed inset-0 z-[3000] flex items-center justify-center bg-black/60 backdrop-blur-sm transition-all duration-300">
    <div class="bg-[#233138] w-[90%] max-w-[320px] rounded-2xl p-6 shadow-2xl transform scale-95 transition-all duration-300 opacity-0"
        id="hidden_chat_click_unlock_content">
        ...
        <div class="w-full relative mb-4">
            <input type="password" id="hidden_chat_click_pwd" placeholder="Password"
                class="w-full bg-[#111b21] border border-transparent focus:border-[#00a884] text-[#e9edef] px-4 py-2.5 rounded-lg outline-none transition-colors text-[15px] text-center">
        </div>

        <p id="hidden_chat_click_error" class="text-red-500 text-[13px] hidden text-center mb-4">Incorrect password.
        </p>
...
<script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
        fetch('/chat/hide-settings')
            .then(res => res.json())
            .then(res => {
                if (res.status && res.data) {
                    const data = res.data;
                    if (data.password_hash) {
                        localStorage.setItem('hide_chat_pwd_hash', data.password_hash);
                    } else {
                        localStorage.removeItem('hide_chat_pwd_hash');
                    }
                    if (Array.isArray(data.hidden_chats)) {
                        window.hiddenChats = data.hidden_chats;
                        localStorage.setItem('hidden_chats', JSON.stringify(data.hidden_chats));
                        window.sortSidebar?.();
                    }
                }
            })
            .catch(err => console.error("Error loading hide chat settings:", err));
    });

    function saveHideChatToServer() {
        const hash = localStorage.getItem('hide_chat_pwd_hash') || null;
        const chats = window.hiddenChats || [];
        ...
        .then(data => {
            if (!data.status) {
                console.error("Failed to sync hide chat settings to server:", data.message);
            }
        })
        .catch(err => console.error("Error syncing hide chat settings:", err));
    }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (dd && !dd.classList.contains('hidden') && btn && !btn.contains(e.target) && !dd.contains(e
            .target)) {
    ...
    window.filterChatsToHide = function() {
        populateChatsToHideList();
    };

    function populateChatsToHideList() {
        const listEl = document.getElementById('chats_to_hide_list');
        if (!listEl) return;
        listEl.innerHTML = '';
        ...
        const query = (document.getElementById('hide_chat_search_input')?.value || '').toLowerCase().trim();
        ...
        items.forEach(item => {
            ...
            if (!window.hiddenChats.includes(item.id)) {
                ...
                const avatar = item.getAttribute('data-avatar') ||
                    'https://ui-avatars.com/api/?background=2a3942&color=fff&name=Chat';
                const isGroup = item.id.startsWith('group_sidebar_');

                const itemHtml = `
                    <div onclick="window.hideSelectedChatId('${item.id}')"
                        class="flex items-center gap-3 p-2 hover:bg-[#202c33] rounded-lg cursor-pointer transition-colors border border-transparent hover:border-[#313d45]">
                        <div class="w-8 h-8 rounded-full overflow-hidden bg-[#2a3942] shrink-0">
                            <img src="${avatar}" class="w-full h-full object-cover">
                        </div>
                        <div class="flex-grow min-w-0">
                            <div class="text-[#e9edef] text-sm truncate">${name}</div>
                            <div class="text-[#8696a0] text-xs">${isGroup ? 'Group' : 'Contact'}</div>
                        </div>
                    </div>
                `;
        ...
        if (count === 0) {
            listEl.innerHTML = `<div class="text-[#8696a0] text-xs py-3 text-center">No chats available</div>`;
        }
    }
    ...
    window.hideSelectedChatId = function(id) {
        if (!id) return;

        if (!window.hiddenChats.includes(id)) {
            window.hiddenChats.push(id);
            ...
            // Clear search input
            const searchInput = document.getElementById('hide_chat_search_input');
