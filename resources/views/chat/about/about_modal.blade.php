        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
            ...
            <div class="space-y-2 relative">
                ...
                <div
                    ...
                    <input type="text" id="about_input" value="{{ auth()->user()->name }}"
                        ...
                        <div class="relative flex-1">
                            ...
                            <div id="duration_dropdown"
                                ...
                                @foreach($durations as $dur)
                                    <button onclick="selectDuration(event, '{{ $dur }}')"
                                        ...
                                        <span class="pointer-events-none">{{ $dur }}</span>
                                        <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"
                                            class="text-[#00a884] duration-check pointer-events-none {{ $dur == 'Until I change it' ? '' : 'hidden' }}"
                                            data-duration="{{ $dur }}">
            ...
            <div class="pt-6 border-t border-[#313d45] space-y-1">
                ...
                @foreach($presets as $preset)
                    <div class="flex items-center gap-4 py-3 px-4 hover:bg-[#202c33] rounded-xl cursor-pointer transition-colors"
                        onclick="setPresetStatus('{{ $preset['text'] }}')">
                        <span class="text-xl">{{ $preset['icon'] }}</span>
                        <span class="text-[#e9edef] text-[15px]">{{ $preset['text'] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
