    ...
    <div class="bg-[#111b21] w-full max-w-md rounded-2xl overflow-hidden shadow-2xl flex flex-col border border-[#313d45]">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="space-y-1">
                ...
                <div class="flex items-center justify-between py-4 px-4 hover:bg-[#202c33] rounded-xl cursor-pointer group transition-colors" onclick="selectAboutPrivacy('{{ $option }}')">
                    <span class="text-[#e9edef] text-[16px]">{{ $option }}</span>
                    <div class="w-5 h-5 rounded-full border-2 {{ $option == 'Nobody' ? 'border-[#00a884] flex items-center justify-center' : 'border-[#8696a0]' }} transition-all privacy-radio" data-option="{{ $option }}">
                        @if($option == 'Nobody')
