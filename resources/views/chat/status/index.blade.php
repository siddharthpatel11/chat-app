    ...
    function updateMyStatusPreview(statuses) {
        ...
        if (statuses && statuses.length > 0) {
            ...
            if (lastStatus.type === 'text') {
                ...
                textPreview.classList.add('hidden');
                const myFallbackAvatar = (lastStatus.userAvatar && lastStatus.userAvatar !== 'undefined') ? lastStatus.userAvatar : "{{ auth()->user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=2a3942&color=fff' }}";
