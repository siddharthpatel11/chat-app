...
<div id="gs_gif_viewer" class="hidden fixed inset-0 z-[2000] bg-black flex flex-col transition-opacity duration-300 opacity-0 pointer-events-auto">
    <!-- Header -->
    <div class="flex items-center justify-between px-4 py-3 bg-gradient-to-b from-black/60 to-transparent absolute top-0 w-full z-[2100]">
        <div class="flex items-center gap-4">
            <!-- Back Button -->
            <button onclick="window.closeGlobalSearchGifViewer()" class="text-white hover:text-[#d1d7db] transition-colors focus:outline-none p-1 rounded-full">
            ...
            <div class="flex flex-col">
                <div id="gs_gif_viewer_sender_name" class="text-white text-[16px] font-medium leading-tight"></div>
                <div id="gs_gif_viewer_time" class="text-white/70 text-[13px] mt-0.5"></div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-5 text-white relative">
            <!-- Status Button -->
            <button id="gs_gif_viewer_btn_status" class="hover:text-[#d1d7db] transition-colors focus:outline-none p-1 rounded-full" title="Set Status">
            ...
            <!-- Download Button -->
            <button id="gs_gif_viewer_btn_download" class="hover:text-[#d1d7db] transition-colors focus:outline-none p-1 rounded-full" title="Download">
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <!-- Forward Button -->
            <button id="gs_gif_viewer_btn_forward" class="hover:text-[#d1d7db] transition-colors focus:outline-none p-1 rounded-full" title="Forward">
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div id="gs_gif_viewer_dropdown_menu" class="hidden absolute top-12 right-0 bg-[#233138] border border-[#313d45] rounded-lg shadow-xl z-[2200] py-2 w-56 transition-opacity duration-150">
                ...
                <button id="gs_gif_viewer_btn_delete" class="w-full text-left px-4 py-3 text-[#f15c6d] hover:bg-[#182229] transition-colors text-[14.5px] flex items-center gap-3">
    ...
    <!-- Main Image -->
    <div class="flex-1 flex items-center justify-center relative overflow-hidden" id="gs_gif_viewer_image_container">
        <img id="gs_gif_viewer_img" src="" class="max-w-full max-h-full object-contain transition-transform duration-200">
    </div>
    ...
    <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-t from-black/80 to-transparent absolute bottom-0 w-full z-[2100]">
        ...
        <div class="flex items-center gap-2">
            <button id="gs_gif_viewer_btn_react" class="text-white hover:text-[#d1d7db] transition-colors focus:outline-none p-2 rounded-full hover:bg-white/10" title="React">
            ...
            <div id="gs_gif_viewer_reactions_container" class="flex gap-1"></div>
        </div>

        <!-- Reply Button -->
        <button id="gs_gif_viewer_btn_reply" class="flex items-center gap-2 text-white hover:bg-white/10 transition-colors focus:outline-none px-4 py-2 rounded-full border border-white/20 bg-black/40 backdrop-blur-sm">
...
<script>
    window.gsGifViewerCurrentContext = null;

    window.openGlobalSearchGifViewer = function(key, chatId, url, senderName, timestampStr, isGroup, text) {
        window.gsGifViewerCurrentContext = { key, chatId, url, senderName, timestampStr, isGroup, text };
        
        document.getElementById('gs_gif_viewer_img').src = url;
        document.getElementById('gs_gif_viewer_sender_name').textContent = senderName;
        document.getElementById('gs_gif_viewer_time').textContent = timestampStr;
        
        // Show existing reactions if available
        const reactionsContainer = document.getElementById('gs_gif_viewer_reactions_container');
        ...
        const viewer = document.getElementById('gs_gif_viewer');
        ...
        if (window.refreshGsGifViewerReactions) {
            window.refreshGsGifViewerReactions();
        }
    };

    window.refreshGsGifViewerReactions = async function() {
        if (!window.gsGifViewerCurrentContext) return;
        const msgId = window.gsGifViewerCurrentContext.key;
        const isGroup = window.gsGifViewerCurrentContext.isGroup;
        let targetChatId = window.gsGifViewerCurrentContext.chatId;
        let path = '';
    ...
    window.closeGlobalSearchGifViewer = function() {
        const viewer = document.getElementById('gs_gif_viewer');
        viewer.classList.remove('show');
        setTimeout(() => {
            viewer.classList.add('hidden');
            document.getElementById('gs_gif_viewer_img').src = '';
            window.gsGifViewerCurrentContext = null;
        }, 300);
        window.closeGsGifViewerMenu();
    };

    // Close on escape key
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && document.getElementById('gs_gif_viewer').classList.contains('show')) {
            window.closeGlobalSearchGifViewer();
        }
    });

    window.toggleGsGifViewerMenu = function(e) {
        if (e) e.stopPropagation();
        const menu = document.getElementById('gs_gif_viewer_dropdown_menu');
        menu.classList.toggle('hidden');
    };

    window.closeGsGifViewerMenu = function() {
        const menu = document.getElementById('gs_gif_viewer_dropdown_menu');
    ...
    document.getElementById('gs_gif_viewer').addEventListener('click', (e) => {
        if (!e.target.closest('#gs_gif_viewer_dropdown_menu') && !e.target.closest('button[onclick="window.toggleGsGifViewerMenu(event)"]')) {
            window.closeGsGifViewerMenu();
        }
    });
    
    // Wire up actions
    document.getElementById('gs_gif_viewer_btn_download').addEventListener('click', () => {
        if (!window.gsGifViewerCurrentContext) return;
        const link = document.createElement('a');
        link.href = window.gsGifViewerCurrentContext.url;
        link.download = `GIF_${window.gsGifViewerCurrentContext.key}.jpg`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    });

    const handleGifForwardOrShare = () => {
        if (!window.gsGifViewerCurrentContext) return;

        window.closeGlobalSearchGifViewer();
        if (window.openForwardModal) {
            window.selectedMessages = new Set([window.gsGifViewerCurrentContext.key]);
            
            if (!window.globalMessages) window.globalMessages = {};
            if (!window.globalMessages[window.gsGifViewerCurrentContext.key]) {
                window.globalMessages[window.gsGifViewerCurrentContext.key] = {
                    key: window.gsGifViewerCurrentContext.key,
                    file_url: window.gsGifViewerCurrentContext.url,
                    type: 'image',
                    text: window.gsGifViewerCurrentContext.text || ''
                };
            }
    ...
    document.getElementById('gs_gif_viewer_btn_status').addEventListener('click', async () => {
        ...
        if (window.openMediaStatusWithFiles) {
            try {
                ...
                const response = await fetch(window.gsGifViewerCurrentContext.url);
                const blob = await response.blob();
                
                let filename = 'status_image.jpg';
                try {
                    const urlPath = new URL(gsGifViewerCurrentContext.url).pathname;
    ...
    document.getElementById('gs_gif_viewer_btn_reply').addEventListener('click', () => {
        if (!gsGifViewerCurrentContext) return;
        window.closeGlobalSearchGifViewer();
        
        const chatId = gsGifViewerCurrentContext.chatId;
        const key = gsGifViewerCurrentContext.key;
        
        let cId = chatId;
        let isGroup = gsGifViewerCurrentContext.isGroup;
        
        if (chatId.startsWith('group_')) {
            cId = chatId.replace('group_', '');
            let name = gsGifViewerCurrentContext.senderName;
            let avatar = '';
        ...
        if (!window.globalMessages[key]) {
            window.globalMessages[key] = {
                key: key,
                file_url: gsGifViewerCurrentContext.url,
                type: 'image',
                text: gsGifViewerCurrentContext.text || ''
            };
        }
        
        const senderName = gsGifViewerCurrentContext.senderName;
        const text = gsGifViewerCurrentContext.text || 'GIF';
        const url = gsGifViewerCurrentContext.url;
    ...
    document.getElementById('gs_gif_viewer_btn_show_chat').addEventListener('click', () => {
        if (!window.gsGifViewerCurrentContext) return;
        window.closeGlobalSearchGifViewer();
        
        const chatId = window.gsGifViewerCurrentContext.chatId;
        let cId = chatId;
        let isGroup = window.gsGifViewerCurrentContext.isGroup;
        if (chatId.startsWith('group_')) {
            cId = chatId.replace('group_', '');
        } else {
            const ids = chatId.replace('chat_', '').split('_');
        ...
        let name = window.gsGifViewerCurrentContext.senderName;
        let avatar = '';
        let status = 'online';
        ...
        if (window.jumpToMessage) {
            setTimeout(() => window.jumpToMessage(chatId, window.gsGifViewerCurrentContext.key), 500);
        }
    });

    document.getElementById('gs_gif_viewer_btn_delete').addEventListener('click', () => {
        if (!gsGifViewerCurrentContext) return;
        window.closeGlobalSearchGifViewer();
        if (window.deleteMsg) {
            // Need globalMessages context
            if (!window.globalMessages) window.globalMessages = {};
            if (!window.globalMessages[gsGifViewerCurrentContext.key]) {
                window.globalMessages[gsGifViewerCurrentContext.key] = {
                    key: gsGifViewerCurrentContext.key,
                    file_url: gsGifViewerCurrentContext.url,
                    type: 'image',
                    sender_id: gsGifViewerCurrentContext.isGroup ? null : (gsGifViewerCurrentContext.senderName === 'You' ? window.myUserId : 'other')
                };
            }
            window.deleteMsg(gsGifViewerCurrentContext.key);
        }
    });

    // Handle double click to zoom
    document.getElementById('gs_gif_viewer_image_container').addEventListener('dblclick', (e) => {
