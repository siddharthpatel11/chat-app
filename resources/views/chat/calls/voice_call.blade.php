<x-app-layout>
    @push('styles')
        ...
        <style>
            ...
            .dot-1 { animation: dot-blink 1.4s ease-in-out infinite; }
            .dot-2 { animation: dot-blink 1.4s ease-in-out 0.2s infinite; }
            .dot-3 { animation: dot-blink 1.4s ease-in-out 0.4s infinite; }
            ...
            .call-control:hover { transform: scale(1.1); background: rgba(255,255,255,0.25); }
            .call-control:active { transform: scale(0.95); }
            .call-control.active { background: white; color: #111b21; }
    ...
    <div class="call-bg w-full h-screen flex flex-col items-center justify-between font-['Inter'] select-none" id="voice_call_screen">
        ...
        <div class="flex-1 flex flex-col items-center justify-center -mt-12 fade-in-up-delay">
            <div class="relative flex items-center justify-center mb-8">
                ...
                <div class="w-[140px] h-[140px] rounded-full overflow-hidden border-[3px] border-[#00a884]/30 shadow-2xl relative z-10 bg-[#202c33]">
                    <img id="call_user_avatar" src="{!! $avatar !!}" class="w-full h-full object-cover" alt="User avatar">
                </div>
            </div>
            ...
            <div id="call_status" class="flex items-center gap-1.5">
                <span id="call_status_text" class="text-[#8696a0] text-[15px] font-normal">{{ $role === 'caller' ? 'Calling' : 'Connecting' }}</span>
        ...
        <div class="pb-16 w-full max-w-md px-8 fade-in-up-delay-2">
            <div class="flex items-center justify-between px-6 mb-8">
                <button id="speaker_btn" onclick="toggleSpeaker()" class="call-control w-14 h-14 rounded-full bg-white/10 text-white flex items-center justify-center">
                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/>
                    </svg>
                </button>
                <button id="mute_btn" onclick="toggleMute()" class="call-control w-14 h-14 rounded-full bg-white/10 text-white flex items-center justify-center">
                ...
                <button class="call-control w-14 h-14 rounded-full bg-white/10 text-white flex items-center justify-center opacity-50 cursor-not-allowed">
            ...
            <div class="flex justify-center">
                <button onclick="endCall()" class="end-call-btn w-16 h-16 rounded-full bg-red-500 hover:bg-red-600 text-white flex items-center justify-center transition-all duration-300 active:scale-90">
                    <svg class="w-8 h-8 rotate-[135deg]" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M20 15.5c-1.2 0-2.4-.2-3.6-.6-.3-.1-.7 0-1 .2l-2.2 2.2c-2.8-1.4-5.1-3.8-6.6-6.6l2.2-2.2c.3-.3.4-.7.2-1-.3-1.1-.5-2.3-.5-3.5 0-.6-.4-1-1-1H5c-.6 0-1 .4-1 1 0 9.4 7.6 17 17 17 .6 0 1-.4 1-1v-3.5c0-.6-.4-1-1-1z"/>
                    </svg>
                </button>
            </div>
        </div>
    ...
    <script type="module">
        ...
        async function startAsCallee() {
            ...
            if (callData) otherUserId = callData.caller_id;
