        ...
        <div class="py-2">
            <div id="add_group_contacts_list">
                @foreach($users ?? [] as $user)
                    ...
                    @endphp
                    <div onclick="window.toggleGroupMemberSelection({{ $user->id }}, '{{ addslashes($user->name ?: $user->phone) }}', '{{ $userAvatar }}')"
                        class="flex items-center px-3 py-3 hover:bg-[#202c33] cursor-pointer transition-colors group-member-item" data-name="{{ strtolower($user->name ?: $user->phone) }}" id="group_member_row_{{ $user->id }}">
